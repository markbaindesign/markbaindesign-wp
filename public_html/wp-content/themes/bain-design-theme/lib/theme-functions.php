...
if (!function_exists('bd324_show_latest_posts')) :
   function bd324_show_latest_posts($post_type = 'post', $number_of_posts = '1')
   {
      ...
      if ($query->have_posts()) :
         ...
         $content = '';
         $content .= '<div class="posts__wrapper posts__wrappercount-' . $number_of_posts . '">';
         $content .= '<div class="media_objects">';
         while ($query->have_posts()) :
            $query->the_post();
            $content .= '<a href="' . get_the_permalink() . '" class="media_object">';
            $content .= '<img src="' . get_the_post_thumbnail_url(get_the_ID()). '">';
            $content .= '<p class="wp-caption-text">' . get_the_title() . '</p>';
            $content .= '</a>';
         endwhile;
